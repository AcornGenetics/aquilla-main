...
This checklist focuses on keeping the existing FastAPI app (`sentri/sentri_web`) while updating the UI to match the new design.

## 1) Inventory the current UI surface
- Identify the static pages served by FastAPI: `/`, `/ready`, `/run`, `/complete` in `sentri/sentri_web/main.py`.
- Review the markup for each screen in `sentri/sentri_web/static/index.html`, `ready.html`, `run.html`, and `complete.html`.
- Review the shared UI behavior in `sentri/sentri_web/static/script.js` (screen routing, button posts, results table, profile dropdown).
...
- Copy visual styles (colors, spacing, fonts) from the new design into `sentri/sentri_web/static/styles.css`.

## 4) Add new pages
- Create new HTML files in `sentri/sentri_web/static/` for each new page (e.g., `settings.html`, `history.html`).
- Add new FastAPI routes in `sentri/sentri_web/main.py` that return those files.
- Update sidebar links to point at the new routes.

## 5) Change the start page
- Update the `/` route in `sentri/sentri_web/main.py` to return the new start page (e.g., `login.html` or `dashboard.html`).
- If you keep `/ready` as the first operational screen, link to it after login.

## 6) Add a login screen (visual + minimal logic)
- Create `login.html` in `sentri/sentri_web/static/` with the desired design.
...
- Update typography, colors, and spacing in `sentri/sentri_web/static/styles.css`.
