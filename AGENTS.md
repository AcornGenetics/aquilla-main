...
- `sentri_lib/` holds the core hardware-control drivers and helpers (motors, TEC, lid heater, state requests).
- `sentri_curve/` contains PCR curve analysis and results processing utilities.
- `sentri_web/` is the FastAPI backend plus `sentri_web/static/` for the no-build HTML/CSS/JS UI.
...
- `uvicorn main:app --reload --host 127.0.0.1` from `sentri_web/` starts the FastAPI UI backend.
...
- The run page uses `sentri_web/static/script.js` to toggle sections and the Done modal; `/run/complete/ack` resets the server screen to `ready`.
...
- On-screen keyboard is provided by `sentri_web/static/keyboard.js` and wired into Run/Login/Profile edit pages for kiosk use.
...
## Change Log
- Updated `sentri_web/static/keyboard.js` to avoid double input on kiosks by using pointer handling with touch/mouse suppression, and bumped keyboard cache keys to `?v=5` in login/run/profile pages.
- Added global tap highlight suppression and focus-state overrides in `sentri_web/static/styles.css` with cache-key bumps across UI pages to reduce touch flashes.
