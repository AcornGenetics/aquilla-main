...
try:
    ...
    # Load actual config
    config = Config()
    AXIS_POSITIONS = config.axis["positions"]          # e.g. [320, 675, 1040, 1390, 1740, 2080]
    AXIS_HOME_STEPS = config.axis["home_steps"]       # 2500 - used for homing (overshoots, stops at flag)
    
    AXIS_WELL_ONE = AXIS_POSITIONS[0]                  # first position
    AXIS_WELL_SPACING = AXIS_POSITIONS[1] - AXIS_POSITIONS[0]  # derived for display purposes
    
    AXIS_MIN_STEPS = 0                                 # Home position
    AXIS_MAX_STEPS = AXIS_POSITIONS[5] + 100          # a bit past last position
    STEP_DELAY = 0.001
    ...
    AXIS_POSITIONS = [320, 675, 1040, 1390, 1740, 2080]
