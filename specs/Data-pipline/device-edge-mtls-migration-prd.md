...
- **Device sync auth — existing seam (`POST /sync/flush` → `sentri_web.sync`).** Prior art: `tests/unit/test_background_sync.py`, which monkeypatches `sentri_web.sync.requests.post` and captures call kwargs. Replace the existing `TestApiKeyHeader` class (asserts `x-api-key` is sent) with tests asserting the client certificate tuple is passed and **no** `x-api-key` header is present. Keep the existing "no endpoint → synced 0" and "network error swallowed, events stay pending" behaviors green.
