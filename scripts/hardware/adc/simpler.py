...
def print_result ( t0, reply1, reply2, adc_value, tag = "", file = sys.stdout ):

    def my_print( x ): return print ( x, end = " ", file = file )

    my_print ( "%6.3f"%( ( time.time() - t0 ) )           )
    my_print ( ".".join ( [ "%02x"%r for r in reply1])    )
    my_print ( ".".join ( [ "%02x"%r for r in reply2])    )
    my_print ( "%.4f"%adc_value                           )

    m_buf_on  = statistics.mean( buf_on )
    m_buf_off = statistics.mean( buf_off )

    my_print ( "%.4f"%m_buf_on                 )
    my_print ( "%.4f"%statistics.pstdev( buf_on )         )
    my_print ( "%.4f"%m_buf_off                           )
    my_print ( "%.4f"%statistics.pstdev( buf_off )        )
    my_print ( "%.4f"%(m_buf_on -m_buf_off )              )
    print( tag )

try:
    for j in range ( 40 ):
        GPIO.output(pin_27, GPIO.HIGH)  # Turn pin 22 on
        GPIO.output(pin_22, GPIO.LOW)  # Turn pin 22 on
        for i in range ( 10 ):
            time.sleep ( 0.01)
        ...
        GPIO.output(pin_27, GPIO.LOW)   # Turn pin 22 off
        GPIO.output(pin_22, GPIO.HIGH)   # Turn pin 22 off
        for i in range ( 10 ):
            time.sleep ( 0.01 )
            reply1 = spi.xfer2( [ 0x41 ] + [0x00,0x00, ] )
            reply2 = spi.xfer2( [ 0x42 ] + [0x00,0x00, 0x00] )
