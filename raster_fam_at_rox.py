...
# =============================================================================
# CONFIGURATION: FAM LED scanning ROX position range
# =============================================================================
dye = "fam"  # Use FAM LED/sensor
scan_offset = 0  # But use ROX position range (offset=0)

print("#", dye)

log(f"=" * 60)
log(f"RASTER SCAN - {dye.upper()} LED at ROX POSITIONS")
log(f"=" * 60)
log(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
log(f"")
log(f"NOTE: This scan uses FAM LED but scans the ROX position range")
log(f"      to diagnose sensor offset issues.")
log(f"")
...
# Use ROX position range (offset=0) instead of FAM (offset=2)
ax_min = axis.positions[scan_offset] - 100
ax_max = axis.positions[scan_offset + 3] + 100

# Y range stays the same
dr0 = 0
dr1 = drawer.read_steps + 500

log(f"")
log(f"Scan parameters:")
log(f"  LED/Sensor: {dye.upper()}")
log(f"  Position range: ROX (offset={scan_offset})")
...
log(f"")
log(f"  Normal FAM range would be: {axis.positions[2]-100} to {axis.positions[5]+100}")
log(f"  Using ROX range instead:   {ax_min} to {ax_max}")
log(f"")
